 ...
 * Requires PHP:	  7.0
 * Author URI:        https://petermorlion.com
 * Text Domain:       SEPA-QR-for-Woocommerce
 * License:           GPL-2.0+
	...
	if ( !empty($order->get_total()) && (float)$order->get_total() > 0 && $order->get_payment_method() == 'bacs' && ('on-hold' == $order->status || 'pending' == $order->status)) {
		...
		<p>
			...
			if ($store_qr_code_as_image !== 'on') {
				echo esc_attr(__('For a convenient payment scan this qr code! Some email clients unfortunately will not show Base64 encoded images. Sorry for that!' ,'SEPA-QR-for-Woocommerce'));
			} else {
				echo esc_attr(__('For a convenient payment scan this qr code!' ,'SEPA-QR-for-Woocommerce'));
			}
...
function muxp_qrpage_from_hash($template) {
	...
	if ( false === muxp_valid_md5($md5) ) { 
		$html = '<p>' .  __('This is not a valid md5 hash', 'SEPA-QR-for-Woocommerce')  . '<br>' .  __('Generating uncached dummy QR for 11 Euro/OrderID 11', 'SEPA-QR-for-Woocommerce') . '<br><img src="' . muxp_create_qrcode(11,11) . '"></p>';
...
function muxp_create_qrcode ($amount,$orderid) {
 ...
 $payloadtext = __('Order-ID:' , 'SEPA-QR-for-Woocommerce') . ' ' . $orderid;
...
function muxp_add_admin_menu() {
	add_options_page(__('SEPA QR', 'SEPA-QR-for-Woocommerce'), __('SEPA QR', 'SEPA-QR-for-Woocommerce'), 'manage_options', 'muxp_options', 'muxp_settings_page');
}
...
function muxp_admin_init() {
	add_settings_section('muxp_gdpr', __('GDPR', 'SEPA-QR-for-Woocommerce'), 'muxp_settings_callback', 'muxp_settings');
